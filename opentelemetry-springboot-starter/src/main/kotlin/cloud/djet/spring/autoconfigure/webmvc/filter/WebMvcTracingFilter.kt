...
class WebMvcTracingFilter(openTelemetry: OpenTelemetry?) : OncePerRequestFilter(), Ordered {
	...
	companion object {
		...
		val excludeRequests = listOf(
			AntPathMatcher("**/actuator/health/**"),
			AntPathMatcher("**/swagger-ui/*.js"),
			AntPathMatcher("**/swagger-ui/*.css"),
			AntPathMatcher("**/swagger-ui/*.html")
		)
	}

	// we disable tracing for requests that is useless for OpenTelemetry
	override fun shouldNotFilter(request: HttpServletRequest): Boolean {
		return excludeRequests.any { it.isPattern(request.requestURI) }
	}
